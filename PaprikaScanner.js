...
async function scanFlow() {
  ...
  sendNotification("Analyzing Recipe", "Sending photo to vision model...")

  let responseText
  try {
    responseText = await callOpenRouter(apiKey, base64)
  } catch(apiErr) {
    sendNotification("Scan Failed", apiErr.message.substring(0, 100))
  ...
  sendNotification("Recipe Found", recipe.name)
...
// ── Notification Helper (wrapped in try/catch — not all iOS versions support schedule) ──
